...
TEST (wallet, status)
{
	...
	system.nodes[0]->peers.insert (btcb::endpoint (boost::asio::ip::address_v6::loopback (), 10000), 0);
...
TEST (history, short_text)
{
	...
	btcb::ledger ledger (store, system.nodes[0]->stats);
	{
		...
		btcb::keypair key;
		btcb::send_block send (ledger.latest (transaction, btcb::test_genesis_key.pub), btcb::test_genesis_key.pub, 0, btcb::test_genesis_key.prv, btcb::test_genesis_key.pub, 0);
		...
		btcb::change_block change (receive.hash (), key.pub, btcb::test_genesis_key.prv, btcb::test_genesis_key.pub, 0);
...
TEST (wallet, republish)
{
	...
	{
		auto transaction (system.nodes[0]->store.tx_begin (true));
		btcb::send_block block (system.nodes[0]->ledger.latest (transaction, btcb::test_genesis_key.pub), key.pub, 0, btcb::test_genesis_key.prv, btcb::test_genesis_key.pub, 0);
		hash = block.hash ();
...
TEST (wallet, seed_work_generation)
{
	...
	auto transaction (system.wallet (0)->wallets.tx_begin ());
	ASSERT_FALSE (btcb::work_validate (system.nodes[0]->ledger.latest_root (transaction, key1), work));
}
