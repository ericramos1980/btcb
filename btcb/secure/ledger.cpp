...
{
public:
	rollback_visitor (btcb::transaction const & transaction_a, btcb::ledger & ledger_a) :
...
{
public:
	ledger_processor (btcb::ledger &, btcb::transaction const &, bool = false);
			...
			if (result.code == btcb::process_result::progress)
			{
				...
				if (result.code == btcb::process_result::progress)
				{
					if (!is_send)
					{
						...
						{
							result.code = (ledger.store.block_exists (transaction, btcb::block_type::state, block_a.hashables.link) || ledger.store.block_exists (transaction, btcb::block_type::send, block_a.hashables.link)) ? btcb::process_result::progress : btcb::process_result::gap_source; // Have we seen the source block already? (Harmless)
							if (result.code == btcb::process_result::progress)
							{
				...
				if (result.code == btcb::process_result::progress)
				{
					...
					if (result.code == btcb::process_result::progress)
					{
						...
						if (result.code == btcb::process_result::progress)
						{
							...
							result.account = block_a.hashables.account;
							result.amount = 0;
							ledger.store.block_put (transaction, hash, block_a, 0, btcb::epoch::epoch_1);
...
void ledger_processor::receive_block (btcb::receive_block const & block_a)
{
	...
	if (result.code == btcb::process_result::progress)
	{
		...
		if (result.code == btcb::process_result::progress)
		{
			...
			if (result.code == btcb::process_result::progress)
			{
				result.code = (ledger.store.block_exists (transaction, btcb::block_type::send, block_a.hashables.source) || ledger.store.block_exists (transaction, btcb::block_type::state, block_a.hashables.source)) ? btcb::process_result::progress : btcb::process_result::gap_source; // Have we seen the source block already? (Harmless)
				if (result.code == btcb::process_result::progress)
				{
					auto account (ledger.store.frontier_get (transaction, block_a.hashables.previous));
					...
					if (result.code == btcb::process_result::progress)
					{
						result.code = btcb::validate_message (account, hash, block_a.signature) ? btcb::process_result::bad_signature : btcb::process_result::progress; // Is the signature valid (Malformed)
						if (result.code == btcb::process_result::progress)
						{
							btcb::account_info info;
...
void ledger_processor::open_block (btcb::open_block const & block_a)
{
	...
	if (result.code == btcb::process_result::progress)
	{
		auto source_missing (!ledger.store.block_exists (transaction, btcb::block_type::send, block_a.hashables.source) && !ledger.store.block_exists (transaction, btcb::block_type::state, block_a.hashables.source));
		result.code = source_missing ? btcb::process_result::gap_source : btcb::process_result::progress; // Have we seen the source block? (Harmless)
		if (result.code == btcb::process_result::progress)
		{
			result.code = btcb::validate_message (block_a.hashables.account, hash, block_a.signature) ? btcb::process_result::bad_signature : btcb::process_result::progress; // Is the signature valid (Malformed)
			if (result.code == btcb::process_result::progress)
			{
				btcb::account_info info;
				...
				if (result.code == btcb::process_result::progress)
				{
					...
					if (result.code == btcb::process_result::progress)
					{
						result.code = block_a.hashables.account == btcb::burn_account ? btcb::process_result::opened_burn_account : btcb::process_result::progress; // Is it burning 0 account? (Malicious)
						if (result.code == btcb::process_result::progress)
						{
							result.code = pending.epoch == btcb::epoch::epoch_0 ? btcb::process_result::progress : btcb::process_result::unreceivable; // Are we receiving a state-only send? (Malformed)
							if (result.code == btcb::process_result::progress)
...
ledger_processor::ledger_processor (btcb::ledger & ledger_a, btcb::transaction const & transaction_a, bool valid_signature_a) :
ledger (ledger_a),
transaction (transaction_a),
...
btcb::uint128_t btcb::ledger::balance (btcb::transaction const & transaction_a, btcb::block_hash const & hash_a)
{
	btcb::summation_visitor visitor (transaction_a, store);
...
btcb::process_return btcb::ledger::process (btcb::transaction const & transaction_a, btcb::block const & block_a, bool valid_signature)
{
...
bool btcb::ledger::is_send (btcb::transaction const & transaction_a, btcb::state_block const & block_a)
{
	bool result (false);
...
void btcb::ledger::rollback (btcb::transaction const & transaction_a, btcb::block_hash const & block_a)
{
...
btcb::account btcb::ledger::account (btcb::transaction const & transaction_a, btcb::block_hash const & hash_a)
{
	btcb::account result;
	auto hash (hash_a);
	btcb::block_hash successor (1);
	btcb::block_info block_info;
	auto block (store.block_get (transaction_a, hash));
	assert (block);
	while (!successor.is_zero () && block->type () != btcb::block_type::state && store.block_info_get (transaction_a, successor, block_info))
	{
	...
	{
		auto state_block (dynamic_cast<btcb::state_block *> (block.get ()));
...
btcb::checksum btcb::ledger::checksum (btcb::transaction const & transaction_a, btcb::account const & begin_a, btcb::account const & end_a)
{
	btcb::checksum result;
...
void btcb::ledger::checksum_update (btcb::transaction const & transaction_a, btcb::block_hash const & hash_a)
{
	btcb::checksum value;
...
std::shared_ptr<btcb::block> btcb::ledger::successor (btcb::transaction const & transaction_a, btcb::uint256_union const & root_a)
{
