...
wallet (wallet_a)
{
	...
	version = new QLabel (boost::str (boost::format ("Version %1%.%2%") % BTCB_VERSION_MAJOR % BTCB_VERSION_MINOR).c_str ());
...
public:
	...
	void open_block (btcb::open_block const & block_a)
	{
		...
		{
			account = btcb::genesis_account;
			amount = btcb::genesis_amount;
		}
	}
...
active_status (*this)
{
	...
	send_account->setPlaceholderText (btcb::zero_key.pub.to_account ().c_str ());
...
void btcb_qt::wallet::start ()
{
	...
	QObject::connect (send_blocks_send, &QPushButton::released, [this_w]() {
		if (auto this_l = this_w.lock ())
		{
			...
			{
				btcb::uint128_t actual (amount.number () * this_l->rendering_ratio);
	...
	node.observers.endpoint.add ([this_w](btcb::endpoint const &) {
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w]() {
				if (auto this_l = this_w.lock ())
...
{
	auto balance_str = btcb::amount (balance).format_balance (rendering_ratio, 0, false);
...
wallet (wallet_a)
{
	...
	new_representative->setPlaceholderText (btcb::zero_key.pub.to_account ().c_str ());
...
bootstrap_label (new QLabel ("IPV6:port \"::ffff:192.168.0.1:17075\"")),
