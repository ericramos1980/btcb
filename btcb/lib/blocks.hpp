...
template <typename T>
bool read (btcb::stream & stream_a, T & value)
{
...
public:
	...
	virtual btcb::block_hash source () const = 0;
	// Previous block or account number for open blocks
	virtual btcb::block_hash root () const = 0;
	// Link field for state blocks, zero otherwise.
	virtual btcb::block_hash link () const = 0;
	virtual btcb::account representative () const = 0;
...
private:
	std::mutex mutex;
	std::unordered_map<btcb::uint256_union, std::weak_ptr<btcb::block>> blocks;
