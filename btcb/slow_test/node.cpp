	...
	std::vector<boost::thread> threads;
	{
		...
		for (auto i (0); i < 20; ++i)
		{
			threads.push_back (boost::thread ([wallet_l, &key]() {
				...
				{
					wallet_l->send_action (btcb::test_genesis_key.pub, key.pub, 1000);
				}
			}));
		}
		...
		for (auto i (nodes.begin ()), n (nodes.end ()); i != n; ++i)
		{
			switch (*i)
			{
				...
				case 1:
					...
					{
						++message_count;
						auto entry (btcb::random_pool.GenerateWord32 (0, node_count - 1));
						switch (nodes[entry])
						{
							case 0:
								nodes[entry] = 1;
...
TEST (peer_container, random_set)
{
	...
	btcb::peer_container container (btcb::endpoint (loopback, 24000));
	for (auto i (0); i < 200; ++i)
	{
		container.contacted (btcb::endpoint (loopback, 24001 + i), 0);
	}
