...
public:
	...
	opencl_enable (false)
	{
		btcb::random_pool.GenerateBlock (wallet.bytes.data (), wallet.bytes.size ());
...
bool update_config (qt_wallet_config & config_a, boost::filesystem::path const & config_path_a, std::fstream & config_file_a)
{
	...
	auto error (false);
	if (!btcb::fetch_object (config_a, config_path_a))
	{
...
int run_wallet (QApplication & application, int argc, char * const * argv, boost::filesystem::path const & data_path, btcb::node_flags const & flags)
{
	...
	if (!error)
	{
		...
		btcb::work_pool work (config.node.work_threads, opencl ? [&opencl](btcb::uint256_union const & root_a) {
			return opencl->generate_work (root_a);
		}
		                                                       : std::function<boost::optional<uint64_t> (btcb::uint256_union const &)> (nullptr));
		btcb::alarm alarm (io_ctx);
		btcb::node_init init;
		node = std::make_shared<btcb::node> (init, io_ctx, data_path, alarm, config.node, work);
		if (!init.error ())
		{
			...
			btcb::thread_runner runner (io_ctx, node->config.io_threads);
