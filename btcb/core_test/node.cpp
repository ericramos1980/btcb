...
TEST (node, block_store_path_failure)
{
	...
	auto node (std::make_shared<btcb::node> (init, *service, 0, path, alarm, logging, work));
...
TEST (node, DISABLED_fork_stale)
{
	...
	node2.bootstrap_initiator.bootstrap (node1.network.endpoint ());
	node2.peers.rep_response (node1.network.endpoint (), btcb::test_genesis_key.pub, btcb::genesis_amount);
	btcb::genesis genesis;
	btcb::keypair key1;
	btcb::keypair key2;
...
TEST (node, no_voting)
{
	...
	ASSERT_EQ (0, node1.stats.count (btcb::stat::type::message, btcb::stat::detail::confirm_ack, btcb::stat::dir::in));
}
