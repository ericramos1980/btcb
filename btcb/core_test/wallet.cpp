...
TEST (wallet, no_key)
{
	...
	ASSERT_FALSE (init);
	btcb::transaction transaction (env.tx_begin (true));
	btcb::kdf kdf;
...
TEST (wallet, work)
{
	...
	while (!done)
	{
		btcb::transaction transaction (system.nodes[0]->store.tx_begin ());
		uint64_t work (0);
...
TEST (wallet, work_generate)
{
	...
	while (again)
	{
		ASSERT_NO_ERROR (system.poll ());
		auto transaction (system.nodes[0]->store.tx_begin ());
		again = wallet->store.work_get (transaction, account1, work1) || btcb::work_validate (system.nodes[0]->ledger.latest_root (transaction, account1), work1);
	}
}
...
TEST (wallet, version_3_upgrade)
{
	...
	btcb::random_pool.GenerateBlock (seed.data.bytes.data (), seed.data.bytes.size ());
	btcb::raw_key password_l;
	btcb::wallet_value value (wallet->store.entry_get_raw (transaction, btcb::wallet_store::wallet_key_special));
	btcb::raw_key kdf;
