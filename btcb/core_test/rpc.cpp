...
{
public:
	test_response (boost::property_tree::ptree const & request_a, btcb::rpc & rpc_a, boost::asio::io_context & io_ctx) :
	request (request_a),
	sock (io_ctx),
	status (0)
	{
		sock.async_connect (btcb::tcp_endpoint (boost::asio::ip::address_v6::loopback (), rpc_a.config.port), [this](boost::system::error_code const & ec) {
			if (!ec)
			{
...
TEST (rpc, account_balance)
{
	...
	btcb::rpc rpc (system.io_ctx, *system.nodes[0], btcb::rpc_config (true));
	rpc.start ();
...
TEST (rpc, account_weight)
{
	...
	btcb::rpc rpc (system.io_ctx, node1, btcb::rpc_config (true));
	rpc.start ();
	boost::property_tree::ptree request;
...
TEST (rpc, frontier)
{
	...
	std::unordered_map<btcb::account, btcb::block_hash> source;
	{
		...
		for (auto i (0); i < 1000; ++i)
		{
			...
			system.nodes[0]->store.account_put (transaction, key.pub, btcb::account_info (key.prv.data, 0, 0, 0, 0, 0, btcb::epoch::epoch_0));
		}
	}
	btcb::keypair key;
	btcb::rpc rpc (system.io_ctx, *system.nodes[0], btcb::rpc_config (true));
...
TEST (rpc, history)
{
	...
	btcb::genesis genesis;
	btcb::state_block usend (btcb::genesis_account, node0->latest (btcb::genesis_account), btcb::genesis_account, btcb::genesis_amount - btcb::Gbcb_ratio, btcb::genesis_account, btcb::test_genesis_key.prv, btcb::test_genesis_key.pub, 0);
	btcb::state_block ureceive (btcb::genesis_account, usend.hash (), btcb::genesis_account, btcb::genesis_amount, usend.hash (), btcb::test_genesis_key.prv, btcb::test_genesis_key.pub, 0);
	btcb::state_block uchange (btcb::genesis_account, ureceive.hash (), btcb::keypair ().pub, btcb::genesis_amount, 0, btcb::test_genesis_key.prv, btcb::test_genesis_key.pub, 0);
	{
	...
	btcb::rpc rpc (system.io_ctx, *node0, btcb::rpc_config (true));
	rpc.start ();
	boost::property_tree::ptree request;
...
{
	btcb::system system (24000, 2);
	system.nodes[0]->peers.insert (btcb::endpoint (boost::asio::ip::address_v6::from_string ("::ffff:80.80.80.80"), 4000), btcb::protocol_version);
	btcb::rpc rpc (system.io_ctx, *system.nodes[0], btcb::rpc_config (true));
	rpc.start ();
	boost::property_tree::ptree request;
...
TEST (rpc, pending)
{
	...
	btcb::block_hash hash1 (blocks_node.begin ()->second.get<std::string> (""));
...
TEST (rpc, search_pending)
{
	...
	btcb::send_block block (system.nodes[0]->latest (btcb::test_genesis_key.pub), btcb::test_genesis_key.pub, btcb::genesis_amount - system.nodes[0]->config.receive_minimum.number (), btcb::test_genesis_key.prv, btcb::test_genesis_key.pub, 0);
...
TEST (rpc, version)
{
	...
	ASSERT_EQ (boost::str (boost::format ("Btcb %1%.%2%") % BTCB_VERSION_MAJOR % BTCB_VERSION_MINOR), response1.json.get<std::string> ("node_vendor"));
...
TEST (rpc, work_generate)
{
	...
	uint64_t work2;
	ASSERT_FALSE (btcb::from_string_hex (work1, work2));
	ASSERT_FALSE (btcb::work_validate (hash1, work2));
}
...
TEST (rpc, work_peer_many)
{
	...
	config2.port += 0;
	btcb::rpc rpc2 (system2.io_ctx, node2, config2);
	rpc2.start ();
	btcb::rpc_config config3 (true);
	config3.port += 1;
	btcb::rpc rpc3 (system3.io_ctx, node3, config3);
	rpc3.start ();
	btcb::rpc_config config4 (true);
	config4.port += 2;
	btcb::rpc rpc4 (system4.io_ctx, node4, config4);
	rpc4.start ();
...
TEST (rpc, bootstrap)
{
	...
	btcb::rpc rpc (system0.io_ctx, *system0.nodes[0], btcb::rpc_config (true));
	rpc.start ();
	boost::property_tree::ptree request;
...
TEST (rpc, block_create_state_open)
{
	...
	request.put ("work", btcb::to_string_hex (system.nodes[0]->work_generate_blocking (send_block->hash ())));
	btcb::rpc rpc (system.io_ctx, *system.nodes[0], btcb::rpc_config (true));
	rpc.start ();
	test_response response (request, rpc, system.io_ctx);
	system.deadline_set (5s);
...
TEST (rpc, online_reps)
{
	...
	system.wallet (0)->send_action (btcb::test_genesis_key.pub, btcb::test_genesis_key.pub, btcb::Gbcb_ratio);
	system.deadline_set (10s);
	while (system.nodes[1]->online_reps.online_stake () == system.nodes[1]->config.online_weight_minimum.number ())
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	btcb::rpc rpc (system.io_ctx, *system.nodes[1], btcb::rpc_config (true));
	rpc.start ();
	boost::property_tree::ptree request;
	...
	ASSERT_EQ (btcb::test_genesis_key.pub.to_account (), item->first);
	...
	auto send (system.wallet (1)->send_action (btcb::test_genesis_key.pub, new_rep, system.nodes[0]->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send);
	auto receive (system.wallet (1)->receive_action (static_cast<btcb::send_block &> (*send), new_rep, system.nodes[0]->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, receive);
	auto change (system.wallet (1)->change_action (btcb::test_genesis_key.pub, new_rep));
	ASSERT_NE (nullptr, change);
	system.deadline_set (5s);
...
TEST (rpc, confirmation_history)
{
	btcb::system system (24000, 1);
	system.wallet (0)->insert_adhoc (btcb::test_genesis_key.prv);
	auto block (system.wallet (0)->send_action (btcb::test_genesis_key.pub, btcb::test_genesis_key.pub, btcb::Gbcb_ratio));
...
TEST (rpc, node_id)
{
	...
	btcb::keypair node_id (system.nodes[0]->store.get_node_id (transaction));
