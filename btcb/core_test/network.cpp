...
TEST (network, keepalive_ipv4)
{
	...
	node1->start ();
	system.nodes.push_back (node1);
	node1->send_keepalive (btcb::endpoint (boost::asio::ip::address_v4::loopback (), 24000));
	auto initial (system.nodes[0]->stats.count (btcb::stat::type::message, btcb::stat::detail::keepalive, btcb::stat::dir::in));
	system.deadline_set (10s);
	while (system.nodes[0]->stats.count (btcb::stat::type::message, btcb::stat::detail::keepalive, btcb::stat::dir::in) == initial)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	node1->stop ();
...
TEST (network, send_insufficient_work)
{
	...
	btcb::publish publish (std::move (block));
...
TEST (bootstrap_processor, lazy_hash)
{
	...
	auto node1 (std::make_shared<btcb::node> (init1, system.io_ctx, 24001, btcb::unique_path (), system.alarm, system.logging, system.work));
	node1->peers.insert (system.nodes[0]->network.endpoint (), btcb::protocol_version);
...
{
	ASSERT_FALSE (btcb::reserved_address (btcb::endpoint (boost::asio::ip::address_v6::from_string ("2001::"), 0), true));
	btcb::endpoint loopback (boost::asio::ip::address_v6::from_string ("::1"), 1);
	ASSERT_FALSE (btcb::reserved_address (loopback, false));
	ASSERT_TRUE (btcb::reserved_address (loopback, true));
}
...
{
	btcb::stat stats;
	btcb::udp_buffer buffer (stats, 512, 1);
...
{
	btcb::stat stats;
	btcb::udp_buffer buffer (stats, 512, 2);
	auto buffer1 (buffer.allocate ());
	ASSERT_NE (nullptr, buffer1);
...
{
	btcb::stat stats;
	btcb::udp_buffer buffer (stats, 512, 16);
