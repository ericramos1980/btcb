	...
	if (ec == btcb::error_cli::unknown_command)
	{
		...
		else if (vm.count ("debug_bootstrap_generate"))
		{
			...
			if (key_it != vm.end ())
			{
				...
				if (!key.decode_hex (key_it->second.as<std::string> ()))
				{
					...
					btcb::work_pool work (std::numeric_limits<unsigned>::max (), nullptr);
		...
		else if (vm.count ("debug_profile_kdf"))
		{
			...
			while (true)
			{
				...
				auto success (argon2_hash (1, btcb::wallet_store::kdf_work, 1, password.data (), password.size (), salt.bytes.data (), salt.bytes.size (), result.bytes.data (), result.bytes.size (), NULL, 0, Argon2_d, 0x10));
				(void)success;
				...
				if (!result)
				{
					...
					if (!error)
					{
						...
						if (!error)
						{
							...
							auto opencl (btcb::opencl_work::create (true, { platform, device, threads }, logging));
							btcb::work_pool work_pool (std::numeric_limits<unsigned>::max (), opencl ? [&opencl](btcb::uint256_union const & root_a) {
		...
		else if (vm.count ("debug_profile_process"))
		{
			if (btcb::btcb_network == btcb::btcb_networks::btcb_test_network)
			{
				...
				btcb::block_hash genesis_latest (node->latest (btcb::test_genesis_key.pub));
				...
				for (auto i (0); i != num_accounts; ++i)
				{
					...
					auto send (std::make_shared<btcb::state_block> (btcb::test_genesis_key.pub, genesis_latest, btcb::test_genesis_key.pub, genesis_balance, keys[i].pub, btcb::test_genesis_key.prv, btcb::test_genesis_key.pub, work.generate (genesis_latest)));
					genesis_latest = send->hash ();
					blocks.push_back (std::move (send));
					auto open (std::make_shared<btcb::state_block> (keys[i].pub, 0, keys[i].pub, balances[i], genesis_latest, keys[i].prv, keys[i].pub, work.generate (keys[i].pub)));
				...
				for (auto i (0); i != num_interations; ++i)
				{
					for (auto j (0); j != num_accounts; ++j)
					{
						...
						--balances[j];
						auto send (std::make_shared<btcb::state_block> (keys[j].pub, frontiers[j], keys[j].pub, balances[j], keys[other].pub, keys[j].prv, keys[j].pub, work.generate (frontiers[j])));
						frontiers[j] = send->hash ();
						blocks.push_back (std::move (send));
						// Receiving
						++balances[other];
						auto receive (std::make_shared<btcb::state_block> (keys[other].pub, frontiers[other], keys[other].pub, balances[other], frontiers[j], keys[other].prv, keys[other].pub, work.generate (frontiers[other])));
			...
			{
				std::cerr << "For this test ACTIVE_NETWORK should be btcb_test_network" << std::endl;
			}
		}
		else if (vm.count ("debug_profile_votes"))
		{
			if (btcb::btcb_network == btcb::btcb_networks::btcb_test_network)
			{
				...
				for (auto i (0); i != num_representatives; ++i)
				{
					...
					btcb::state_block send (btcb::test_genesis_key.pub, genesis_latest, btcb::test_genesis_key.pub, genesis_balance, keys[i].pub, btcb::test_genesis_key.prv, btcb::test_genesis_key.pub, work.generate (genesis_latest));
					genesis_latest = send.hash ();
					node->ledger.process (transaction, send);
					btcb::state_block open (keys[i].pub, 0, keys[i].pub, balance, genesis_latest, keys[i].prv, keys[i].pub, work.generate (keys[i].pub));
				...
				for (auto i (0); i != num_elections; ++i)
				{
					...
					auto send (std::make_shared<btcb::state_block> (btcb::test_genesis_key.pub, genesis_latest, btcb::test_genesis_key.pub, genesis_balance, destination.pub, btcb::test_genesis_key.prv, btcb::test_genesis_key.pub, work.generate (genesis_latest)));
					genesis_latest = send->hash ();
		...
		else if (vm.count ("debug_validate_blocks"))
		{
			...
			for (auto i (node.node->store.latest_begin (transaction)), n (node.node->store.latest_end ()); i != n; ++i)
			{
				...
				while (!hash.is_zero ())
				{
					...
					if ((block->type () == btcb::block_type::open && block->root () != account) || (block->type () == btcb::block_type::state && static_cast<btcb::state_block const &> (*block.get ()).hashables.account != account))
					{
		...
		{
			std::cout << "Version " << BTCB_VERSION_MAJOR << "." << BTCB_VERSION_MINOR << std::endl;
		}
		else
		{
