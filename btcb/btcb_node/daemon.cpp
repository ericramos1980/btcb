...
btcb_daemon::daemon_config::daemon_config () :
rpc_enable (false),
opencl_enable (false)
{
}

void btcb_daemon::daemon_config::serialize_json (boost::property_tree::ptree & tree_a)
{
...
bool btcb_daemon::daemon_config::deserialize_json (bool & upgraded_a, boost::property_tree::ptree & tree_a)
{
	auto error (false);
	try
	{
...
bool btcb_daemon::daemon_config::upgrade_json (unsigned version_a, boost::property_tree::ptree & tree_a)
{
	tree_a.put ("version", std::to_string (json_version));
	auto result (false);
	switch (version_a)
...
void btcb_daemon::daemon::run (boost::filesystem::path const & data_path, btcb::node_flags const & flags)
{
	...
	btcb_daemon::daemon_config config;
	btcb::set_secure_perm_directory (data_path, error_chmod);
	auto config_path ((data_path / "config.json"));
	std::unique_ptr<btcb::thread_runner> runner;
	auto error (btcb::fetch_object (config, config_path));
	btcb::set_secure_perm_file (config_path, error_chmod);
	if (!error)
	{
		config.node.logging.init (data_path);
		boost::asio::io_context io_ctx;
		auto opencl (btcb::opencl_work::create (config.opencl_enable, config.opencl, config.node.logging));
		btcb::work_pool opencl_work (config.node.work_threads, opencl ? [&opencl](btcb::uint256_union const & root_a) {
		...
		try
		{
			auto node (std::make_shared<btcb::node> (init, io_ctx, data_path, alarm, config.node, opencl_work));
			if (!init.error ())
			{
				node->flags = flags;
				node->start ();
				std::unique_ptr<btcb::rpc> rpc = get_rpc (io_ctx, *node, config.rpc);
