...
#include <btcb/node/rpc_secure.hpp>
#endif

#include <btcb/lib/errors.hpp>

btcb::rpc_secure_config::rpc_secure_config () :
enable (false),
verbose_logging (false)
{
}

void btcb::rpc_secure_config::serialize_json (boost::property_tree::ptree & tree_a) const
{
...
bool btcb::rpc_secure_config::deserialize_json (boost::property_tree::ptree const & tree_a)
{
	auto error (false);
	try
	{
...
void btcb::rpc_config::serialize_json (boost::property_tree::ptree & tree_a) const
{
...
bool btcb::rpc_config::deserialize_json (boost::property_tree::ptree const & tree_a)
{
	auto result (false);
	try
	{
...
btcb::rpc::rpc (boost::asio::io_context & io_ctx_a, btcb::node & node_a, btcb::rpc_config const & config_a) :
acceptor (io_ctx_a),
config (config_a),
node (node_a)
{
...
void btcb::rpc::start ()
{
	auto endpoint (btcb::tcp_endpoint (config.address, config.port));
	...
	acceptor.listen ();
	node.observers.blocks.add ([this](std::shared_ptr<btcb::block> block_a, btcb::account const & account_a, btcb::uint128_t const &, bool) {
...
{
	auto connection (std::make_shared<btcb::rpc_connection> (node, *this));
...
btcb::rpc_handler::rpc_handler (btcb::node & node_a, btcb::rpc & rpc_a, std::string const & body_a, std::string const & request_id_a, std::function<void(boost::property_tree::ptree const &)> const & response_a) :
body (body_a),
...
void btcb::rpc::observer_action (btcb::account const & account_a)
{
	std::shared_ptr<btcb::payment_observer> observer;
	{
...
void btcb::error_response (std::function<void(boost::property_tree::ptree const &)> response_a, std::string const & message_a)
{
...
void btcb::rpc_handler::response_errors ()
{
...
std::shared_ptr<btcb::wallet> btcb::rpc_handler::wallet_impl ()
{
	if (!ec)
	{
...
btcb::account btcb::rpc_handler::account_impl (std::string account_text)
{
...
btcb::amount btcb::rpc_handler::amount_impl ()
{
...
btcb::block_hash btcb::rpc_handler::hash_impl (std::string search_text)
{
...
btcb::amount btcb::rpc_handler::threshold_optional_impl ()
{
	btcb::amount result (0);
...
uint64_t btcb::rpc_handler::work_optional_impl ()
{
	uint64_t result (0);
...
uint64_t btcb::rpc_handler::count_impl ()
{
	uint64_t result (0);
	if (!ec)
	{
...
uint64_t btcb::rpc_handler::count_optional_impl (uint64_t result)
{
...
bool btcb::rpc_handler::rpc_control_impl ()
{
	bool result (false);
	if (!ec)
	{
		...
		{
			ec = btcb::error_rpc::rpc_control_disabled;
		}
		else
		{
			result = true;
...
void btcb::rpc_handler::account_balance ()
{
...
void btcb::rpc_handler::account_block_count ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void btcb::rpc_handler::account_create ()
{
	rpc_control_impl ();
...
void btcb::rpc_handler::account_get ()
{
...
void btcb::rpc_handler::account_info ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void btcb::rpc_handler::account_key ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void btcb::rpc_handler::account_list ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void btcb::rpc_handler::account_move ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	if (!ec)
...
void btcb::rpc_handler::account_remove ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	auto account (account_impl ());
...
void btcb::rpc_handler::account_representative ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void btcb::rpc_handler::account_representative_set ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	auto account (account_impl ());
...
void btcb::rpc_handler::account_weight ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void btcb::rpc_handler::accounts_balances ()
{
...
void btcb::rpc_handler::accounts_create ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
...
void btcb::rpc_handler::accounts_frontiers ()
{
...
void btcb::rpc_handler::accounts_pending ()
{
	...
	for (auto & accounts : request.get_child ("accounts"))
	{
		...
		if (!ec)
		{
			...
			for (auto i (node.store.pending_begin (transaction, btcb::pending_key (account, 0))); btcb::pending_key (i->first).account == account && peers_l.size () < count; ++i)
			{
				...
				std::shared_ptr<btcb::block> block (include_active ? nullptr : node.store.block_get (transaction, key.hash));
...
void btcb::rpc_handler::available_supply ()
{
	auto genesis_balance (node.balance (btcb::genesis_account)); // Cold storage genesis
	...
	auto available (btcb::genesis_amount - genesis_balance - landing_balance - faucet_balance - burned_balance);
	response_l.put ("available", available.convert_to<std::string> ());
	response_errors ();
}

void btcb::rpc_handler::block ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
...
void btcb::rpc_handler::block_confirm ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
...
void btcb::rpc_handler::blocks_info ()
{
	const bool pending = request.get<bool> ("pending", false);
	const bool source = request.get<bool> ("source", false);
...
void btcb::rpc_handler::block_account ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
...
void btcb::rpc_handler::block_count ()
{
	auto transaction (node.store.tx_begin_read ());
...
void btcb::rpc_handler::block_count_type ()
{
	auto transaction (node.store.tx_begin_read ());
...
void btcb::rpc_handler::block_create ()
{
	rpc_control_impl ();
	if (!ec)
	{
...
void btcb::rpc_handler::block_hash ()
{
...
void btcb::rpc_handler::bootstrap ()
{
...
void btcb::rpc_handler::bootstrap_any ()
{
...
void btcb::rpc_handler::bootstrap_lazy ()
{
	rpc_control_impl ();
	auto hash (hash_impl ());
 ...
 */
void btcb::rpc_handler::bootstrap_status ()
{
...
void btcb::rpc_handler::chain (bool successors)
{
...
void btcb::rpc_handler::confirmation_active ()
{
...
void btcb::rpc_handler::confirmation_history ()
{
	boost::property_tree::ptree elections;
...
void btcb::rpc_handler::confirmation_info ()
{
...
void btcb::rpc_handler::confirmation_quorum ()
{
...
void btcb::rpc_handler::delegators ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void btcb::rpc_handler::delegators_count ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void btcb::rpc_handler::deterministic_key ()
{
...
void btcb::rpc_handler::frontiers ()
{
...
void btcb::rpc_handler::account_count ()
{
	auto transaction (node.store.tx_begin_read ());
...
{
public:
	history_visitor (btcb::rpc_handler & handler_a, bool raw_a, btcb::transaction & transaction_a, boost::property_tree::ptree & tree_a, btcb::block_hash const & hash_a) :
	handler (handler_a),
	raw (raw_a),
	...
	void open_block (btcb::open_block const & block_a)
	{
		...
		{
			tree.put ("account", btcb::genesis_account.to_account ());
			tree.put ("amount", btcb::genesis_amount.convert_to<std::string> ());
		}
	}
...
void btcb::rpc_handler::account_history ()
{
...
void btcb::rpc_handler::keepalive ()
{
	rpc_control_impl ();
	if (!ec)
	{
...
void btcb::rpc_handler::key_create ()
{
	btcb::keypair pair;
...
void btcb::rpc_handler::key_expand ()
{
	std::string key_text (request.get<std::string> ("key"));
	btcb::uint256_union prv;
...
void btcb::rpc_handler::mrai_from_raw (btcb::uint128_t ratio)
{
...
void btcb::rpc_handler::mrai_to_raw (btcb::uint128_t ratio)
{
	auto amount (amount_impl ());
	if (!ec)
	{
 ...
 * @warning This is an internal/diagnostic RPC, do not rely on its interface being stable
 */
void btcb::rpc_handler::node_id_delete ()
{
	rpc_control_impl ();
	if (!ec)
	{
...
void btcb::rpc_handler::password_change ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	if (!ec)
...
void btcb::rpc_handler::password_enter ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void btcb::rpc_handler::password_valid (bool wallet_locked)
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void btcb::rpc_handler::pending_exists ()
{
	auto hash (hash_impl ());
	const bool include_active = request.get<bool> ("include_active", false);
	if (!ec)
...
void btcb::rpc_handler::payment_begin ()
{
...
void btcb::rpc_handler::payment_init ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void btcb::rpc_handler::payment_end ()
{
	auto account (account_impl ());
	auto wallet (wallet_impl ());
	if (!ec)
...
void btcb::rpc_handler::payment_wait ()
{
	...
	if (!ec)
	{
		...
		{
			{
				auto observer (std::make_shared<btcb::payment_observer> (response, rpc, account, amount));
...
void btcb::rpc_handler::process ()
{
	std::string block_text (request.get<std::string> ("block"));
	boost::property_tree::ptree block_l;
	std::stringstream block_stream (block_text);
	boost::property_tree::read_json (block_stream, block_l);
	std::shared_ptr<btcb::block> block (btcb::deserialize_block_json (block_l));
	if (block != nullptr)
	{
	...
	if (!ec)
	{
		...
		if (wallet->store.valid_password (transaction))
		{
			if (wallet->store.find (transaction, account) != wallet->store.end ())
			{
				...
				if (block != nullptr)
				{
					if (node.store.pending_exists (transaction, btcb::pending_key (account, hash)))
					{
						auto work (work_optional_impl ());
						if (!ec && work)
						{
						...
						{
							auto response_a (response);
							wallet->receive_async (std::move (block), account, btcb::genesis_amount, [response_a](std::shared_ptr<btcb::block> block_a) {
								btcb::uint256_union hash_a (0);
								if (block_a != nullptr)
								{
									hash_a = block_a->hash ();
								}
...
void btcb::rpc_handler::receive_minimum ()
{
	rpc_control_impl ();
	if (!ec)
	{
...
void btcb::rpc_handler::receive_minimum_set ()
{
	rpc_control_impl ();
	auto amount (amount_impl ());
	if (!ec)
...
void btcb::rpc_handler::representatives ()
{
	auto count (count_optional_impl ());
	if (!ec)
	{
...
void btcb::rpc_handler::representatives_online ()
{
...
void btcb::rpc_handler::republish ()
{
...
void btcb::rpc_handler::search_pending ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	if (!ec)
...
void btcb::rpc_handler::search_pending_all ()
{
	rpc_control_impl ();
	if (!ec)
	{
			...
			if (!destination.decode_account (destination_text))
			{
				...
				if (!ec)
				{
					...
					wallet->send_async (source, destination, amount.number (), [balance, amount, response_a](std::shared_ptr<btcb::block> block_a) {
						if (block_a != nullptr)
						{
							btcb::uint256_union hash (block_a->hash ());
							boost::property_tree::ptree response_l;
							response_l.put ("block", hash.to_string ());
							response_a (response_l);
						}
...
void btcb::rpc_handler::unchecked ()
{
	auto count (count_optional_impl ());
	if (!ec)
	{
...
void btcb::rpc_handler::unchecked_clear ()
{
	rpc_control_impl ();
	if (!ec)
	{
...
void btcb::rpc_handler::unchecked_get ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
...
void btcb::rpc_handler::unchecked_keys ()
{
	auto count (count_optional_impl ());
...
void btcb::rpc_handler::version ()
{
	response_l.put ("rpc_version", "1");
	response_l.put ("store_version", std::to_string (node.store_version ()));
	response_l.put ("protocol_version", std::to_string (btcb::protocol_version));
	response_l.put ("node_vendor", boost::str (boost::format ("Btcb %1%.%2%") % BTCB_VERSION_MAJOR % BTCB_VERSION_MINOR));
	response_errors ();
}

void btcb::rpc_handler::validate_account_number ()
{
...
void btcb::rpc_handler::wallet_add ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	if (!ec)
...
void btcb::rpc_handler::wallet_add_watch ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	if (!ec)
...
void btcb::rpc_handler::wallet_info ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void btcb::rpc_handler::wallet_balances ()
{
	auto wallet (wallet_impl ());
	auto threshold (threshold_optional_impl ());
	if (!ec)
	{
		...
		{
			btcb::account account (i->first);
			btcb::uint128_t balance = node.ledger.account_balance (transaction, account);
			if (balance >= threshold.number ())
			{
				boost::property_tree::ptree entry;
				btcb::uint128_t pending = node.ledger.account_pending (transaction, account);
				entry.put ("balance", balance.convert_to<std::string> ());
...
void btcb::rpc_handler::wallet_change_seed ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	if (!ec)
...
void btcb::rpc_handler::wallet_contains ()
{
	auto account (account_impl ());
	auto wallet (wallet_impl ());
	if (!ec)
...
void btcb::rpc_handler::wallet_create ()
{
	rpc_control_impl ();
	if (!ec)
	{
...
void btcb::rpc_handler::wallet_destroy ()
{
	rpc_control_impl ();
	if (!ec)
	{
...
void btcb::rpc_handler::wallet_export ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void btcb::rpc_handler::wallet_frontiers ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void btcb::rpc_handler::wallet_key_valid ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void btcb::rpc_handler::wallet_ledger ()
{
	const bool representative = request.get<bool> ("representative", false);
	const bool weight = request.get<bool> ("weight", false);
	const bool pending = request.get<bool> ("pending", false);
...
void btcb::rpc_handler::wallet_lock ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	if (!ec)
...
void btcb::rpc_handler::wallet_pending ()
{
	auto wallet (wallet_impl ());
	auto count (count_optional_impl ());
	auto threshold (threshold_optional_impl ());
	...
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			btcb::account account (i->first);
			boost::property_tree::ptree peers_l;
			for (auto ii (node.store.pending_begin (transaction, btcb::pending_key (account, 0))); btcb::pending_key (ii->first).account == account && peers_l.size () < count; ++ii)
			{
...
void btcb::rpc_handler::wallet_representative ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void btcb::rpc_handler::wallet_representative_set ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	if (!ec)
...
void btcb::rpc_handler::wallet_republish ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	auto count (count_impl ());
...
void btcb::rpc_handler::wallet_work_get ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	if (!ec)
...
void btcb::rpc_handler::work_generate ()
{
	rpc_control_impl ();
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		auto callback = [rpc_l](boost::optional<uint64_t> const & work_a) {
			...
			{
				boost::property_tree::ptree response_l;
				response_l.put ("work", btcb::to_string_hex (work_a.value ()));
...
void btcb::rpc_handler::work_cancel ()
{
	rpc_control_impl ();
	auto hash (hash_impl ());
	if (!ec)
...
void btcb::rpc_handler::work_get ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	auto account (account_impl ());
...
void btcb::rpc_handler::work_set ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	auto account (account_impl ());
...
void btcb::rpc_handler::work_validate ()
{
	auto hash (hash_impl ());
	auto work (work_optional_impl ());
	if (!ec)
	{
		auto validate (btcb::work_validate (hash, work));
		response_l.put ("valid", validate ? "0" : "1");
	}
	response_errors ();
}

void btcb::rpc_handler::work_peer_add ()
{
	rpc_control_impl ();
	if (!ec)
	{
...
void btcb::rpc_handler::work_peers ()
{
	rpc_control_impl ();
	if (!ec)
	{
...
void btcb::rpc_handler::work_peers_clear ()
{
	rpc_control_impl ();
	if (!ec)
	{
...
btcb::rpc_connection::rpc_connection (btcb::node & node_a, btcb::rpc & rpc_a) :
node (node_a.shared ()),
rpc (rpc_a),
...
void btcb::rpc_connection::write_result (std::string body, unsigned version)
{
	...
	boost::beast::http::async_read (socket, buffer, request, [this_l](boost::system::error_code const & ec, size_t bytes_transferred) {
		if (!ec)
		{
			this_l->node->background ([this_l]() {
				...
				{
					auto handler (std::make_shared<btcb::rpc_handler> (*this_l->node, this_l->rpc, this_l->request.body (), request_id, response_handler));
...
void btcb::rpc_handler::process_request ()
{
	try
	{
		...
		else
		{
			...
			{
				mrai_from_raw (btcb::kbcb_ratio);
			}
			else if (action == "krai_to_raw")
			{
				mrai_to_raw (btcb::kbcb_ratio);
			}
			...
			else if (action == "btcb_from_raw")
			{
				mrai_from_raw (btcb::bcb_ratio);
			}
			else if (action == "btcb_to_raw")
			{
				mrai_to_raw (btcb::bcb_ratio);
			}
...
btcb::payment_observer::payment_observer (std::function<void(boost::property_tree::ptree const &)> const & response_a, btcb::rpc & rpc_a, btcb::account const & account_a, btcb::amount const & amount_a) :
rpc (rpc_a),
account (account_a),
amount (amount_a),
response (response_a)
...
void btcb::payment_observer::start (uint64_t timeout)
{
	auto this_l (shared_from_this ());
...
btcb::payment_observer::~payment_observer ()
{
}

void btcb::payment_observer::observe ()
{
...
void btcb::payment_observer::complete (btcb::payment_status status)
{
...
std::unique_ptr<btcb::rpc> btcb::get_rpc (boost::asio::io_context & io_ctx_a, btcb::node & node_a, btcb::rpc_config const & config_a)
{
