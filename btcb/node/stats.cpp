...
bool btcb::stat_config::deserialize_json (boost::property_tree::ptree & tree_a)
{
	bool error = false;
...
std::unique_ptr<btcb::stat_log_sink> btcb::stat::log_sink_json ()
{
	return std::make_unique<json_writer> ();
}

std::unique_ptr<btcb::stat_log_sink> log_sink_file (std::string filename)
{
...
std::string btcb::stat::type_to_string (uint32_t key)
{
	...
	switch (type)
	{
		...
		case btcb::stat::type::traffic:
			res = "traffic";
			break;
		case btcb::stat::type::traffic_bootstrap:
