...
std::chrono::seconds constexpr btcb::node::period;
std::chrono::seconds constexpr btcb::node::cutoff;
std::chrono::seconds constexpr btcb::node::syn_cookie_cutoff;
std::chrono::minutes constexpr btcb::node::backup_interval;
std::chrono::seconds constexpr btcb::node::search_pending_interval;
int constexpr btcb::port_mapping::mapping_timeout;
int constexpr btcb::port_mapping::check_timeout;
unsigned constexpr btcb::active_transactions::announce_interval_ms;
...
namespace btcb
{
extern unsigned char btcb_bootstrap_weights[];
...
socket (node_a.io_ctx, btcb::endpoint (boost::asio::ip::address_v6::any (), port)),
...
void btcb::network::receive ()
{
	...
	socket.async_receive_from (boost::asio::buffer (data->buffer, btcb::network::buffer_size), data->endpoint, [this, data](boost::system::error_code const & error, size_t size_a) {
		if (!error && this->on)
		{
			data->size = size_a;
...
void btcb::network::process_packets ()
{
	while (on)
	{
...
void btcb::network::send_keepalive (btcb::endpoint const & endpoint_a)
{
	...
	send_buffer (bytes->data (), bytes->size (), endpoint_a, [bytes, node_w, endpoint_a](boost::system::error_code const & ec, size_t) {
		if (auto node_l = node_w.lock ())
		{
			...
			{
				node_l->stats.inc (btcb::stat::type::message, btcb::stat::detail::keepalive, btcb::stat::dir::out);
			}
		}
	});
}
...
void btcb::node::keepalive (std::string const & address_a, uint16_t port_a)
{
	...
	network.resolver.async_resolve (boost::asio::ip::udp::resolver::query (address_a, std::to_string (port_a)), [node_l, address_a, port_a](boost::system::error_code const & ec, boost::asio::ip::udp::resolver::iterator i_a) {
		if (!ec)
		{
			...
			{
				node_l->send_keepalive (btcb::map_endpoint_to_v6 (i->endpoint ()));
			}
		}
		else
		{
...
void btcb::network::send_node_id_handshake (btcb::endpoint const & endpoint_a, boost::optional<btcb::uint256_union> const & query, boost::optional<btcb::uint256_union> const & respond_to)
{
	assert (endpoint_a.address ().is_v6 ());
	...
	node.stats.inc (btcb::stat::type::message, btcb::stat::detail::node_id_handshake, btcb::stat::dir::out);
	std::weak_ptr<btcb::node> node_w (node.shared ());
	send_buffer (bytes->data (), bytes->size (), endpoint_a, [bytes, node_w, endpoint_a](boost::system::error_code const & ec, size_t) {
		if (auto node_l = node_w.lock ())
		{
...
void btcb::network::republish (btcb::block_hash const & hash_a, std::shared_ptr<std::vector<uint8_t>> buffer_a, btcb::endpoint endpoint_a)
{
	...
	send_buffer (buffer_a->data (), buffer_a->size (), endpoint_a, [buffer_a, node_w, endpoint_a](boost::system::error_code const & ec, size_t size) {
		if (auto node_l = node_w.lock ())
		{
			...
			{
				node_l->stats.inc (btcb::stat::type::message, btcb::stat::detail::publish, btcb::stat::dir::out);
			}
		}
	});
}
...
bool confirm_block (btcb::transaction const & transaction_a, btcb::node & node_a, T & list_a, std::shared_ptr<btcb::block> block_a, bool also_publish)
{
	...
	{
		node_a.wallets.foreach_representative (transaction_a, [&result, &block_a, &list_a, &node_a, &transaction_a, also_publish](btcb::public_key const & pub_a, btcb::raw_key const & prv_a) {
			result = true;
...
bool confirm_block (btcb::transaction const & transaction_a, btcb::node & node_a, btcb::endpoint & peer_a, std::shared_ptr<btcb::block> block_a, bool also_publish)
{
	std::array<btcb::endpoint, 1> endpoints;
	endpoints[0] = peer_a;
	auto result (confirm_block (transaction_a, node_a, endpoints, std::move (block_a), also_publish));
	return result;
}

void btcb::network::republish_block (std::shared_ptr<btcb::block> block)
{
...
void btcb::network::republish_block_batch (std::deque<std::shared_ptr<btcb::block>> blocks_a, unsigned delay_a)
{
...
void btcb::network::republish_vote (std::shared_ptr<btcb::vote> vote_a)
{
	btcb::confirm_ack confirm (vote_a);
...
{
	auto list (std::make_shared<std::vector<btcb::peer_information>> (node.peers.representatives (std::numeric_limits<size_t>::max ())));
	if (list->empty () || node.peers.total_weight () < node.config.online_weight_minimum.number ())
	{
		// broadcast request to all peers
		list = std::make_shared<std::vector<btcb::peer_information>> (node.peers.list_vector (100));
	}
...
void btcb::network::broadcast_confirm_req_base (std::shared_ptr<btcb::block> block_a, std::shared_ptr<std::vector<btcb::peer_information>> endpoints_a, unsigned delay_a, bool resumption)
{
...
void btcb::network::broadcast_confirm_req_batch (std::deque<std::pair<std::shared_ptr<btcb::block>, std::shared_ptr<std::vector<btcb::peer_information>>>> deque_a, unsigned delay_a)
{
...
void btcb::network::send_confirm_req (btcb::endpoint const & endpoint_a, std::shared_ptr<btcb::block> block)
{
	...
	std::weak_ptr<btcb::node> node_w (node.shared ());
	node.stats.inc (btcb::stat::type::message, btcb::stat::detail::confirm_req, btcb::stat::dir::out);
...
template <typename T>
void rep_query (btcb::node & node_a, T const & peers_a)
{
	auto transaction (node_a.store.tx_begin_read ());
	std::shared_ptr<btcb::block> block (node_a.store.block_random (transaction));
	auto hash (block->hash ());
	...
	std::weak_ptr<btcb::node> node_w (node_a.shared ());
...
void rep_query (btcb::node & node_a, btcb::endpoint const & peers_a)
{
	std::array<btcb::endpoint, 1> peers;
	peers[0] = peers_a;
...
{
public:
	network_message_visitor (btcb::node & node_a, btcb::endpoint const & sender_a) :
	node (node_a),
	sender (sender_a)
	{
	}
	...
	void keepalive (btcb::keepalive const & message_a) override
	{
		...
		{
			auto endpoint_l (btcb::map_endpoint_to_v6 (sender));
	...
	void bulk_pull_blocks (btcb::bulk_pull_blocks const &) override
	{
		assert (false);
	}
	...
	btcb::node & node;
	btcb::endpoint sender;
};
}

void btcb::network::receive_action (btcb::udp_data * data_a)
{
	...
	else if (btcb::reserved_address (data_a->endpoint, false) && !node.config.allow_local_peers)
	{
		allowed_sender = false;
	}
	if (allowed_sender)
...
btcb::node_init::node_init () :
block_store_init (false),
wallet_init (false)
{
}

bool btcb::node_init::error ()
{
...
void btcb::vote_processor::process_loop ()
{
	...
	while (!stopped)
	{
		if (!votes.empty ())
		{
			std::deque<std::pair<std::shared_ptr<btcb::vote>, btcb::endpoint>> votes_l;
			votes_l.swap (votes);
...
void btcb::vote_processor::vote (std::shared_ptr<btcb::vote> vote_a, btcb::endpoint endpoint_a)
{
	assert (endpoint_a.address ().is_v6 ());
	std::unique_lock<std::mutex> lock (mutex);
	if (!stopped)
	{
		...
		if (btcb::btcb_network != btcb::btcb_networks::btcb_test_network)
		{
			// Level 0 (< 0.1%)
...
void btcb::vote_processor::verify_votes (std::deque<std::pair<std::shared_ptr<btcb::vote>, btcb::endpoint>> & votes_a)
{
	...
	btcb::signature_check_set check = { size, messages.data (), lengths.data (), pub_keys.data (), signatures.data (), verifications.data (), &promise };
...
btcb::vote_code btcb::vote_processor::vote_blocking (btcb::transaction const & transaction_a, std::shared_ptr<btcb::vote> vote_a, btcb::endpoint endpoint_a, bool validated)
{
	assert (endpoint_a.address ().is_v6 ());
...
void btcb::signature_checker::add (btcb::signature_check_set & check_a)
{
	{
		std::lock_guard<std::mutex> lock (mutex);
...
void btcb::signature_checker::verify (btcb::signature_check_set & check_a)
{
	...
	auto code (btcb::validate_message_batch (check_a.messages, check_a.message_lengths, check_a.pub_keys, check_a.signatures, check_a.size, check_a.verifications));
	(void)code;
...
void btcb::signature_checker::run ()
{
...
node (node_a),
generator (node_a, btcb::btcb_network == btcb::btcb_networks::btcb_test_network ? std::chrono::milliseconds (10) : std::chrono::milliseconds (500))
{
}
...
void btcb::block_processor::add (std::shared_ptr<btcb::block> block_a, std::chrono::steady_clock::time_point origination)
{
	if (!btcb::work_validate (block_a->root (), block_a->block_work ()))
	{
		{
			std::lock_guard<std::mutex> lock (mutex);
			if (blocks_hashes.find (block_a->hash ()) == blocks_hashes.end ())
			{
				if (block_a->type () == btcb::block_type::state && !node.ledger.is_epoch_link (block_a->link ()))
				{
	...
	{
		BOOST_LOG (node.log) << "btcb::block_processor::add called for hash " << block_a->hash ().to_string () << " with invalid work " << btcb::to_string_hex (block_a->block_work ());
...
void btcb::block_processor::verify_state_blocks (std::unique_lock<std::mutex> & lock_a, size_t max_count)
{
	assert (!mutex.try_lock ());
	auto start_time (std::chrono::steady_clock::now ());
	std::deque<std::pair<std::shared_ptr<btcb::block>, std::chrono::steady_clock::time_point>> items;
	...
	{
		auto & block (static_cast<btcb::state_block &> (*items[i].first));
...
void btcb::block_processor::process_receive_many (std::unique_lock<std::mutex> & lock_a)
{
	lock_a.lock ();
	auto start_time (std::chrono::steady_clock::now ());
	...
	while ((!blocks.empty () || !forced.empty ()) && std::chrono::steady_clock::now () - start_time < node.config.block_processor_batch_max_time)
	{
		...
		std::pair<std::shared_ptr<btcb::block>, std::chrono::steady_clock::time_point> block;
		bool force (false);
		if (forced.empty ())
		{
			block = blocks.front ();
		...
		bool validated_state_block (!force && block.first->type () == btcb::block_type::state);
...
btcb::process_return btcb::block_processor::process_receive_one (btcb::transaction const & transaction_a, std::shared_ptr<btcb::block> block_a, std::chrono::steady_clock::time_point origination, bool validated_state_block)
{
...
void btcb::block_processor::queue_unchecked (btcb::transaction const & transaction_a, btcb::block_hash const & hash_a)
{
	...
	{
		node.store.unchecked_del (transaction_a, btcb::unchecked_key (hash_a, (*i)->hash ()));
...
btcb::node::node (btcb::node_init & init_a, boost::asio::io_context & io_ctx_a, boost::filesystem::path const & application_path_a, btcb::alarm & alarm_a, btcb::node_config const & config_a, btcb::work_pool & work_a) :
io_ctx (io_ctx_a),
config (config_a),
alarm (alarm_a),
work (work_a),
store_impl (std::make_unique<btcb::mdb_store> (init_a.block_store_init, application_path_a / "data.ldb", config_a.lmdb_max_dbs)),
store (*store_impl),
gap_cache (*this),
...
vote_uniquer (block_uniquer)
{
	...
	peers.peer_observer = [this](btcb::endpoint const & endpoint_a) {
	...
	observers.endpoint.add ([this](btcb::endpoint const & endpoint_a) {
		this->network.send_keepalive (endpoint_a);
		rep_query (*this, endpoint_a);
	});
	observers.vote.add ([this](btcb::transaction const & transaction, std::shared_ptr<btcb::vote> vote_a, btcb::endpoint const & endpoint_a) {
		assert (endpoint_a.address ().is_v6 ());
	...
	BOOST_LOG (log) << "Node starting, version: " << BTCB_VERSION_MAJOR << "." << BTCB_VERSION_MINOR;
	...
	if (!init_a.error ())
	{
		...
		node_id = btcb::keypair (store.get_node_id (transaction));
		BOOST_LOG (log) << "Node ID: " << node_id.pub.to_account ();
	}
	peers.online_weight_minimum = config.online_weight_minimum.number ();
	if (btcb::btcb_network == btcb::btcb_networks::btcb_live_network || btcb::btcb_network == btcb::btcb_networks::btcb_beta_network)
	{
		...
		if (!btcb::read (weight_stream, block_height))
		{
			...
			if (ledger.store.block_count (transaction).sum () < max_blocks)
			{
				...
				while (true)
				{
					...
					if (btcb::read (weight_stream, account.bytes))
					{
						break;
					}
					btcb::amount weight;
					if (btcb::read (weight_stream, weight.bytes))
					{
						break;
					}
					BOOST_LOG (log) << "Using bootstrap rep weight: " << account.to_account () << " -> " << weight.format_balance (Mbcb_ratio, 0, true) << " BCB";
...
void btcb::node::send_keepalive (btcb::endpoint const & endpoint_a)
{
	network.send_keepalive (btcb::map_endpoint_to_v6 (endpoint_a));
}
...
void btcb::gap_cache::add (btcb::transaction const & transaction_a, std::shared_ptr<btcb::block> block_a)
{
	auto hash (block_a->hash ());
	std::lock_guard<std::mutex> lock (mutex);
	...
	{
		blocks.get<1> ().modify (existing, [](btcb::gap_information & info) {
			info.arrival = std::chrono::steady_clock::now ();
		});
	}
	else
	{
		blocks.insert ({ std::chrono::steady_clock::now (), hash, std::unordered_set<btcb::account> () });
		if (blocks.size () > max)
		{
		...
		if (existing != blocks.get<1> ().end ())
		{
			...
			if (is_new)
			{
				...
				if (start_bootstrap)
				{
					...
					auto now (std::chrono::steady_clock::now ());
					node.alarm.add (btcb::btcb_network == btcb::btcb_networks::btcb_test_network ? now + std::chrono::milliseconds (5) : now + std::chrono::seconds (5), [node_l, hash]() {
...
void btcb::network::confirm_send (btcb::confirm_ack const & confirm_a, std::shared_ptr<std::vector<uint8_t>> bytes_a, btcb::endpoint const & endpoint_a)
{
	if (node.config.logging.network_publish_logging ())
	{
	...
	node.network.send_buffer (bytes_a->data (), bytes_a->size (), endpoint_a, [bytes_a, node_w, endpoint_a](boost::system::error_code const & ec, size_t size_a) {
		if (auto node_l = node_w.lock ())
		{
			...
			{
				node_l->stats.inc (btcb::stat::type::message, btcb::stat::detail::confirm_ack, btcb::stat::dir::out);
			}
		}
	});
}
...
void btcb::node::keepalive_preconfigured (std::vector<std::string> const & peers_a)
{
	for (auto i (peers_a.begin ()), n (peers_a.end ()); i != n; ++i)
	{
		keepalive (*i, btcb::network::node_port);
	}
}
...
void btcb::node::ongoing_keepalive ()
{
...
void btcb::node::ongoing_syn_cookie_cleanup ()
{
...
void btcb::node::ongoing_rep_crawl ()
{
	auto now (std::chrono::steady_clock::now ());
...
public:
	...
	void success (std::string const & body_a, boost::asio::ip::address const & address)
	{
		...
		try
		{
			...
			{
				if (!btcb::work_validate (root, work))
				{
					set_once (work);
					stop ();
				}
...
btcb::uint128_t btcb::node::delta ()
{
...
void btcb::node::process_confirmed (std::shared_ptr<btcb::block> block_a)
{
	auto hash (block_a->hash ());
...
void btcb::node::process_message (btcb::message & message_a, btcb::endpoint const & sender_a)
{
...
btcb::endpoint btcb::network::endpoint ()
{
	...
	return btcb::endpoint (boost::asio::ip::address_v6::loopback (), port);
}
...
btcb::online_reps::online_reps (btcb::node & node) :
node (node)
{
}

void btcb::online_reps::vote (std::shared_ptr<btcb::vote> const & vote_a)
{
	...
	while (current != reps.end () && current->last_heard + std::chrono::seconds (btcb::node::cutoff) < now)
	{
	...
	auto info (btcb::rep_last_heard_info{ now, rep });
	if (rep_it == reps.get<1> ().end ())
	{
		...
		{
			// overflow
			online_stake_total = std::numeric_limits<btcb::uint128_t>::max ();
		}
		reps.insert (info);
	}
	else
...
void btcb::online_reps::recalculate_stake ()
{
	std::lock_guard<std::mutex> lock (mutex);
	online_stake_total = 0;
	auto transaction (node.store.tx_begin_read ());
...
btcb::uint128_t btcb::online_reps::online_stake ()
{
	std::lock_guard<std::mutex> lock (mutex);
...
bool btcb::reserved_address (btcb::endpoint const & endpoint_a, bool blacklist_loopback)
{
	assert (endpoint_a.address ().is_v6 ());
	...
	else if (btcb::btcb_network == btcb::btcb_networks::btcb_live_network)
	{
...
void btcb::network::send_buffer (uint8_t const * data_a, size_t size_a, btcb::endpoint const & endpoint_a, std::function<void(boost::system::error_code const &, size_t)> callback_a)
{
	std::unique_lock<std::mutex> lock (socket_mutex);
	if (node.config.logging.network_packet_logging ())
	{
	...
	socket.async_send_to (boost::asio::buffer (data_a, size_a), endpoint_a, [this, callback_a](boost::system::error_code const & ec, size_t size_a) {
		...
		this->node.stats.add (btcb::stat::type::traffic, btcb::stat::dir::out, size_a);
		if (ec == boost::system::errc::host_unreachable)
		{
			this->node.stats.inc (btcb::stat::type::error, btcb::stat::detail::unreachable_host, btcb::stat::dir::out);
		}
		if (this->node.config.logging.network_packet_logging ())
		{
...
btcb::election_vote_result::election_vote_result () :
replay (false),
processed (false)
{
}
...
announcements (0)
{
	last_votes.insert (std::make_pair (btcb::not_an_account, btcb::vote_info{ std::chrono::steady_clock::now (), 0, block_a->hash () }));
...
void btcb::election::confirm_once (btcb::transaction const & transaction_a)
{
...
void btcb::election::confirm_back (btcb::transaction const & transaction_a)
{
	std::vector<btcb::block_hash> hashes = { status.winner->previous (), status.winner->source (), status.winner->link () };
...
bool btcb::election::have_quorum (btcb::tally_t const & tally_a, btcb::uint128_t tally_sum)
{
	bool result = false;
...
void btcb::election::log_votes (btcb::tally_t const & tally_a)
{
...
btcb::election_vote_result btcb::election::vote (btcb::account rep, uint64_t sequence, btcb::block_hash block_hash)
{
	...
	if (btcb::btcb_network == btcb::btcb_networks::btcb_test_network || weight > supply / 1000) // 0.1% or above
	{
...
void btcb::active_transactions::announce_votes (std::unique_lock<std::mutex> & lock_a)
{
	std::unordered_set<btcb::block_hash> inactive;
	auto transaction (node.store.tx_begin_read ());
	unsigned unconfirmed_count (0);
	unsigned unconfirmed_announcements (0);
	std::deque<std::shared_ptr<btcb::block>> rebroadcast_bundle;
	std::deque<std::pair<std::shared_ptr<btcb::block>, std::shared_ptr<std::vector<btcb::peer_information>>>> confirm_req_bundle;
	...
	for (auto i (roots.get<1> ().begin ()), n (roots.get<1> ().end ()); i != n; ++i)
	{
		...
		else
		{
			if (i->election->announcements > announcement_long)
			{
				...
				if (i->election->announcements % announcement_long == 1 && roots_size < 100 && btcb::btcb_network != btcb::btcb_networks::btcb_test_network)
				{
			...
			{
				auto reps (std::make_shared<std::vector<btcb::peer_information>> (node.peers.representatives (std::numeric_limits<size_t>::max ())));
				std::unordered_set<btcb::account> probable_reps;
				btcb::uint128_t total_weight (0);
				...
				{
					// broadcast request to all peers
					confirm_req_bundle.push_back (std::make_pair (i->election->status.winner, std::make_shared<std::vector<btcb::peer_information>> (node.peers.list_vector (100))));
				}
			}
		}
...
void btcb::active_transactions::announce_loop ()
{
	std::unique_lock<std::mutex> lock (mutex);
	started = true;
...
bool btcb::active_transactions::add (std::shared_ptr<btcb::block> block_a, std::function<void(std::shared_ptr<btcb::block>)> const & confirmation_action_a)
{
	...
	if (!stopped)
	{
		...
		if (existing == roots.end ())
		{
			...
			roots.insert (btcb::conflict_info{ root, difficulty, election });
...
btcb::active_transactions::active_transactions (btcb::node & node_a) :
node (node_a),
started (false),
stopped (false),
thread ([this]() {
	btcb::thread_role::set (btcb::thread_role::name::announce_loop);
	announce_loop ();
})
{
	std::unique_lock<std::mutex> lock (mutex);
...
btcb::inactive_node::inactive_node (boost::filesystem::path const & path, uint16_t peering_port_a) :
path (path),
...
btcb::udp_buffer::udp_buffer (btcb::stat & stats, size_t size, size_t count) :
stats (stats),
free (count),
full (count),
slab (size * count),
...
btcb::udp_data * btcb::udp_buffer::allocate ()
{
	std::unique_lock<std::mutex> lock (mutex);
...
void btcb::udp_buffer::enqueue (btcb::udp_data * data_a)
{
...
btcb::udp_data * btcb::udp_buffer::dequeue ()
{
	std::unique_lock<std::mutex> lock (mutex);
...
void btcb::udp_buffer::release (btcb::udp_data * data_a)
{
	assert (data_a != nullptr);
	{
		std::lock_guard<std::mutex> lock (mutex);
