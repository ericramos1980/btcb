...
btcb::peer_information::peer_information (btcb::endpoint const & endpoint_a, unsigned network_version_a) :
endpoint (endpoint_a),
...
btcb::peer_information::peer_information (btcb::endpoint const & endpoint_a, std::chrono::steady_clock::time_point const & last_contact_a, std::chrono::steady_clock::time_point const & last_attempt_a) :
endpoint (endpoint_a),
ip_address (endpoint_a.address ()),
...
last_rep_response (std::chrono::steady_clock::time_point ()),
rep_weight (0),
network_version (btcb::protocol_version),
node_id ()
{
}

btcb::peer_container::peer_container (btcb::endpoint const & self_a) :
self (self_a),
peer_observer ([](btcb::endpoint const &) {}),
disconnect_observer ([]() {})
{
}

bool btcb::peer_container::contacted (btcb::endpoint const & endpoint_a, unsigned version_a)
{
	auto endpoint_l (btcb::map_endpoint_to_v6 (endpoint_a));
	auto should_handshake (false);
	if (version_a < btcb::node_id_version)
	{
	...
	else if (!known_peer (endpoint_l))
	{
		...
		if (peers.get<btcb::peer_by_ip_addr> ().count (endpoint_l.address ()) < max_peers_per_ip)
		{
...
bool btcb::peer_container::known_peer (btcb::endpoint const & endpoint_a)
{
	std::lock_guard<std::mutex> lock (mutex);
...
std::deque<btcb::endpoint> btcb::peer_container::list_fanout ()
{
	auto peers (random_set (size_sqrt ()));
	std::deque<btcb::endpoint> result;
...
std::deque<btcb::endpoint> btcb::peer_container::list ()
{
	std::deque<btcb::endpoint> result;
	std::lock_guard<std::mutex> lock (mutex);
...
std::map<btcb::endpoint, unsigned> btcb::peer_container::list_version ()
{
	std::map<btcb::endpoint, unsigned> result;
	std::lock_guard<std::mutex> lock (mutex);
	for (auto i (peers.begin ()), j (peers.end ()); i != j; ++i)
	{
		result.insert (std::pair<btcb::endpoint, unsigned> (i->endpoint, i->network_version));
	}
	return result;
}

std::vector<btcb::peer_information> btcb::peer_container::list_vector (size_t count_a)
{
	...
	{
		result.resize (count_a, btcb::peer_information (btcb::endpoint{}, 0));
	}
	return result;
}

btcb::endpoint btcb::peer_container::bootstrap_peer ()
{
	btcb::endpoint result (boost::asio::ip::address_v6::any (), 0);
	std::lock_guard<std::mutex> lock (mutex);
	;
	for (auto i (peers.get<4> ().begin ()), n (peers.get<4> ().end ()); i != n;)
	{
		...
		{
			result = i->endpoint;
			peers.get<4> ().modify (i, [](btcb::peer_information & peer_a) {
...
boost::optional<btcb::uint256_union> btcb::peer_container::assign_syn_cookie (btcb::endpoint const & endpoint)
{
...
bool btcb::peer_container::validate_syn_cookie (btcb::endpoint const & endpoint, btcb::account node_id, btcb::signature sig)
{
	auto ip_addr (endpoint.address ());
	assert (ip_addr.is_v6 ());
	std::unique_lock<std::mutex> lock (syn_cookie_mutex);
...
std::unordered_set<btcb::endpoint> btcb::peer_container::random_set (size_t count_a)
{
	std::unordered_set<btcb::endpoint> result;
...
void btcb::peer_container::random_fill (std::array<btcb::endpoint, 8> & target_a)
{
...
std::vector<btcb::peer_information> btcb::peer_container::representatives (size_t count_a)
{
	std::vector<peer_information> result;
...
void btcb::peer_container::purge_syn_cookies (std::chrono::steady_clock::time_point const & cutoff)
{
...
std::vector<btcb::peer_information> btcb::peer_container::purge_list (std::chrono::steady_clock::time_point const & cutoff)
{
	std::vector<btcb::peer_information> result;
	{
		std::lock_guard<std::mutex> lock (mutex);
		...
		for (auto i (peers.begin ()), n (peers.end ()); i != n; ++i)
		{
			peers.modify (i, [](btcb::peer_information & info) { info.last_attempt = std::chrono::steady_clock::now (); });
		}
...
std::vector<btcb::endpoint> btcb::peer_container::rep_crawl ()
{
	std::vector<btcb::endpoint> result;
...
size_t btcb::peer_container::size ()
{
	std::lock_guard<std::mutex> lock (mutex);
	return peers.size ();
}

size_t btcb::peer_container::size_sqrt ()
{
	auto result (std::ceil (std::sqrt (size ())));
	return result;
}

std::vector<btcb::peer_information> btcb::peer_container::list_probable_rep_weights ()
{
	std::vector<btcb::peer_information> result;
...
btcb::uint128_t btcb::peer_container::total_weight ()
{
...
bool btcb::peer_container::empty ()
{
	return size () == 0;
}

bool btcb::peer_container::not_a_peer (btcb::endpoint const & endpoint_a, bool blacklist_loopback)
{
	bool result (false);
	...
	else if (btcb::reserved_address (endpoint_a, blacklist_loopback))
	{
		result = true;
	}
...
bool btcb::peer_container::rep_response (btcb::endpoint const & endpoint_a, btcb::account const & rep_account_a, btcb::amount const & weight_a)
{
	...
	{
		peers.modify (existing, [weight_a, &updated, rep_account_a](btcb::peer_information & info) {
...
void btcb::peer_container::rep_request (btcb::endpoint const & endpoint_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	auto existing (peers.find (endpoint_a));
	if (existing != peers.end ())
	{
		peers.modify (existing, [](btcb::peer_information & info) {
			info.last_rep_request = std::chrono::steady_clock::now ();
		});
	}
}

bool btcb::peer_container::reachout (btcb::endpoint const & endpoint_a)
{
...
bool btcb::peer_container::insert (btcb::endpoint const & endpoint_a, unsigned version_a)
{
	assert (endpoint_a.address ().is_v6 ());
	auto unknown (false);
	...
	if (!result)
	{
		if (version_a >= btcb::protocol_version_min)
		{
			std::lock_guard<std::mutex> lock (mutex);
			auto existing (peers.find (endpoint_a));
			if (existing != peers.end ())
			...
			{
				unknown = true;
				if (!result && btcb_network != btcb_networks::btcb_test_network)
				{
					auto ip_peers (peers.get<btcb::peer_by_ip_addr> ().count (endpoint_a.address ()));
				...
				if (!result)
				{
					peers.insert (btcb::peer_information (endpoint_a, version_a));
				}
			}
		}
	}
