...
btcb::socket::socket (std::shared_ptr<btcb::node> node_a) :
...
void btcb::socket::async_read (std::shared_ptr<std::vector<uint8_t>> buffer_a, size_t size_a, std::function<void(boost::system::error_code const &, size_t)> callback_a)
{
	...
	boost::asio::async_read (socket_m, boost::asio::buffer (buffer_a->data (), size_a), [this_l, callback_a](boost::system::error_code const & ec, size_t size_a) {
		this_l->node->stats.add (btcb::stat::type::traffic_bootstrap, btcb::stat::dir::in, size_a);
		this_l->stop ();
...
void btcb::socket::async_write (std::shared_ptr<std::vector<uint8_t>> buffer_a, std::function<void(boost::system::error_code const &, size_t)> callback_a)
{
	...
	boost::asio::async_write (socket_m, boost::asio::buffer (buffer_a->data (), buffer_a->size ()), [this_l, callback_a, buffer_a](boost::system::error_code const & ec, size_t size_a) {
		this_l->node->stats.add (btcb::stat::type::traffic_bootstrap, btcb::stat::dir::out, size_a);
		this_l->stop ();
		callback_a (ec, size_a);
	});
}

void btcb::socket::start (std::chrono::steady_clock::time_point timeout_a)
{
	cutoff = timeout_a.time_since_epoch ().count ();
}

void btcb::socket::stop ()
{
	cutoff = std::numeric_limits<uint64_t>::max ();
}
...
btcb::tcp_endpoint btcb::socket::remote_endpoint ()
{
...
btcb::bootstrap_client::bootstrap_client (std::shared_ptr<btcb::node> node_a, std::shared_ptr<btcb::bootstrap_attempt> attempt_a, btcb::tcp_endpoint const & endpoint_a) :
node (node_a),
attempt (attempt_a),
socket (std::make_shared<btcb::socket> (node_a)),
...
void btcb::bootstrap_client::run ()
{
	auto this_l (shared_from_this ());
...
{
	std::unique_ptr<btcb::frontier_req> request (new btcb::frontier_req);
	request->start.clear ();
...
{
	auto this_l (shared_from_this ());
	size_t size_l (sizeof (btcb::uint256_union) + sizeof (btcb::uint256_union));
...
void btcb::frontier_req_client::received_frontier (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		assert (size_a == sizeof (btcb::uint256_union) + sizeof (btcb::uint256_union));
		btcb::account account;
		btcb::bufferstream account_stream (connection->receive_buffer->data (), sizeof (btcb::uint256_union));
		auto error1 (btcb::read (account_stream, account));
		assert (!error1);
		btcb::block_hash latest;
		btcb::bufferstream latest_stream (connection->receive_buffer->data () + sizeof (btcb::uint256_union), sizeof (btcb::uint256_union));
		auto error2 (btcb::read (latest_stream, latest));
		assert (!error2);
		if (count == 0)
		{
			start_time = std::chrono::steady_clock::now ();
...
void btcb::bulk_pull_client::received_type ()
{
	...
	switch (type)
	{
		...
		{
			connection->socket->async_read (connection->receive_buffer, btcb::send_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
		...
		{
			connection->socket->async_read (connection->receive_buffer, btcb::receive_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		...
		{
			connection->socket->async_read (connection->receive_buffer, btcb::open_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		...
		{
			connection->socket->async_read (connection->receive_buffer, btcb::change_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		...
		{
			connection->socket->async_read (connection->receive_buffer, btcb::state_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
...
void btcb::bulk_push_client::send_finished ()
{
	...
	connection->node->stats.inc (btcb::stat::type::bootstrap, btcb::stat::detail::bulk_push, btcb::stat::dir::out);
	if (connection->node->config.logging.network_logging ())
	{
...
void btcb::bootstrap_attempt::run ()
{
	...
	{
		auto k = btcb::random_pool.GenerateWord32 (0, i);
...
void btcb::bootstrap_attempt::populate_connections ()
{
	...
	if (connections < target)
	{
		...
		for (int i = 0; i < delta; i++)
		{
			...
			if (peer != btcb::endpoint (boost::asio::ip::address_v6::any (), 0))
			{
				auto client (std::make_shared<btcb::bootstrap_client> (node, shared_from_this (), btcb::tcp_endpoint (peer.address (), peer.port ())));
				client->run ();
				std::lock_guard<std::mutex> lock (mutex);
...
{
	auto client (std::make_shared<btcb::bootstrap_client> (node, shared_from_this (), btcb::tcp_endpoint (endpoint_a.address (), endpoint_a.port ())));
	client->run ();
}
...
void btcb::bootstrap_attempt::add_pull (btcb::pull_info const & pull)
{
	{
		std::lock_guard<std::mutex> lock (mutex);
...
void btcb::bootstrap_attempt::lazy_pull_flush ()
{
	...
	for (auto & pull_start : lazy_pulls)
	{
		...
		{
			add_pull (btcb::pull_info (pull_start, pull_start, btcb::block_hash (0), lazy_max_pull_blocks));
		}
	}
	lazy_pulls.clear ();
}
...
bool btcb::bootstrap_attempt::process_block (std::shared_ptr<btcb::block> block_a, uint64_t total_blocks, bool block_expected)
{
...
void btcb::bootstrap_initiator::bootstrap (btcb::endpoint const & endpoint_a, bool add_to_peers)
{
	if (add_to_peers)
	{
		node.peers.insert (btcb::map_endpoint_to_v6 (endpoint_a), btcb::protocol_version);
	}
	std::unique_lock<std::mutex> lock (mutex);
	if (!stopped)
	{
...
btcb::bootstrap_listener::bootstrap_listener (boost::asio::io_context & io_ctx_a, uint16_t port_a, btcb::node & node_a) :
acceptor (io_ctx_a),
...
void btcb::bootstrap_listener::accept_connection ()
{
	auto socket (std::make_shared<btcb::socket> (node.shared ()));
		...
		if (!error)
		{
			switch (header.type)
			{
				case btcb::message_type::bulk_pull:
				{
					...
					{
						extended_size = btcb::bulk_pull::extended_parameters_size;
					}
					else
					{
						extended_size = 0;
					...
					auto this_l (shared_from_this ());
					socket->async_read (receive_buffer, sizeof (btcb::uint256_union) + sizeof (btcb::uint256_union) + extended_size, [this_l, header](boost::system::error_code const & ec, size_t size_a) {
				...
				case btcb::message_type::bulk_pull_account:
				{
					...
					auto this_l (shared_from_this ());
					socket->async_read (receive_buffer, sizeof (btcb::uint256_union) + sizeof (btcb::uint128_union) + sizeof (uint8_t), [this_l, header](boost::system::error_code const & ec, size_t size_a) {
						this_l->receive_bulk_pull_account_action (ec, size_a, header);
					});
					break;
				}
				case btcb::message_type::bulk_pull_blocks:
				{
					if (node->config.logging.network_logging ())
					{
					...
					auto this_l (shared_from_this ());
					socket->async_read (receive_buffer, sizeof (btcb::uint256_union) + sizeof (btcb::uint256_union) + sizeof (bulk_pull_blocks_mode) + sizeof (uint32_t), [this_l, header](boost::system::error_code const & ec, size_t size_a) {
				...
				case btcb::message_type::frontier_req:
				{
					...
					auto this_l (shared_from_this ());
					socket->async_read (receive_buffer, sizeof (btcb::uint256_union) + sizeof (uint32_t) + sizeof (uint32_t), [this_l, header](boost::system::error_code const & ec, size_t size_a) {
...
void btcb::bootstrap_server::receive_bulk_pull_account_action (boost::system::error_code const & ec, size_t size_a, btcb::message_header const & header_a)
{
	...
	{
		auto error (false);
		assert (size_a == (sizeof (btcb::uint256_union) + sizeof (btcb::uint128_union) + sizeof (uint8_t)));
		btcb::bufferstream stream (receive_buffer->data (), size_a);
		...
		if (!error)
		{
			if (node->config.logging.bulk_pull_logging ())
			{
				BOOST_LOG (node->log) << boost::str (boost::format ("Received bulk pull account for %1% with a minimum amount of %2%") % request->account.to_account () % btcb::amount (request->minimum_amount).format_balance (btcb::Mbcb_ratio, 10, true));
			}
			add_request (std::unique_ptr<btcb::message> (request.release ()));
			receive ();
		}
...
void btcb::bootstrap_server::receive_bulk_pull_blocks_action (boost::system::error_code const & ec, size_t size_a, btcb::message_header const & header_a)
{
	if (!ec)
	{
		auto error (false);
		btcb::bufferstream stream (receive_buffer->data (), sizeof (btcb::uint256_union) + sizeof (btcb::uint256_union) + sizeof (bulk_pull_blocks_mode) + sizeof (uint32_t));
		std::unique_ptr<btcb::bulk_pull_blocks> request (new btcb::bulk_pull_blocks (error, stream, header_a));
		if (!error)
		{
			if (node->config.logging.bulk_pull_logging ())
			{
...
void btcb::bootstrap_server::receive_frontier_req_action (boost::system::error_code const & ec, size_t size_a, btcb::message_header const & header_a)
{
	...
	{
		auto error (false);
		btcb::bufferstream stream (receive_buffer->data (), sizeof (btcb::uint256_union) + sizeof (uint32_t) + sizeof (uint32_t));
...
public:
	...
	{
		auto response (std::make_shared<btcb::bulk_pull_blocks_server> (connection, std::unique_ptr<btcb::bulk_pull_blocks> (static_cast<btcb::bulk_pull_blocks *> (connection->requests.front ().release ()))));
		response->send_next ();
	}
 ...
 * DEPRECATED
 */
void btcb::bulk_pull_blocks_server::set_params ()
{
	assert (request != nullptr);
}

void btcb::bulk_pull_blocks_server::send_next ()
{
	send_finished ();
}

void btcb::bulk_pull_blocks_server::send_finished ()
{
	send_buffer->clear ();
	send_buffer->push_back (static_cast<uint8_t> (btcb::block_type::not_a_block));
	auto this_l (shared_from_this ());
...
void btcb::bulk_pull_blocks_server::no_block_sent (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		assert (size_a == 1);
...
btcb::bulk_pull_blocks_server::bulk_pull_blocks_server (std::shared_ptr<btcb::bootstrap_server> const & connection_a, std::unique_ptr<btcb::bulk_pull_blocks> request_a) :
connection (connection_a),
request (std::move (request_a)),
send_buffer (std::make_shared<std::vector<uint8_t>> ())
{
