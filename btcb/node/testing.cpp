...
btcb::system::system (uint16_t port_a, size_t count_a) :
alarm (io_ctx),
work (1, nullptr)
{
	...
	for (size_t i (0); i < count_a; ++i)
	{
		...
		auto node (std::make_shared<btcb::node> (init, io_ctx, btcb::unique_path (), alarm, config, work));
		assert (!init.error ());
		node->start ();
	...
	auto iterations1 (0);
	while (std::any_of (nodes.begin (), nodes.end (), [](std::shared_ptr<btcb::node> const & node_a) { return node_a->bootstrap_initiator.in_progress (); }))
	{
		poll ();
		++iterations1;
...
btcb::landing_store::landing_store ()
{
}
