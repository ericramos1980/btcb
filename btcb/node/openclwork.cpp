...
__kernel void btcb_work (__global ulong * attempt, __global ulong * result_a, __global uchar * item_a)
{
...
btcb::opencl_config::opencl_config () :
platform (0),
device (0),
threads (1024 * 1024)
{
...
void btcb::opencl_config::serialize_json (boost::property_tree::ptree & tree_a) const
{
...
bool btcb::opencl_config::deserialize_json (boost::property_tree::ptree const & tree_a)
{
	auto result (false);
	try
	{
...
btcb::opencl_work::opencl_work (bool & error_a, btcb::opencl_config const & config_a, btcb::opencl_environment & environment_a, btcb::logging & logging_a) :
config (config_a),
context (0),
attempt_buffer (0),
result_buffer (0),
...
logging (logging_a)
{
	...
	if (!error_a)
	{
		...
		if (!error_a)
		{
			btcb::random_pool.GenerateBlock (reinterpret_cast<uint8_t *> (rand.s.data ()), rand.s.size () * sizeof (decltype (rand.s)::value_type));
...
boost::optional<uint64_t> btcb::opencl_work::generate_work (btcb::uint256_union const & root_a)
{
	...
	while (btcb::work_validate (root_a, result) && !error)
	{
		result = rand.next ();
...
std::unique_ptr<btcb::opencl_work> btcb::opencl_work::create (bool create_a, btcb::opencl_config const & config_a, btcb::logging & logging_a)
{
	...
	if (create_a)
	{
		...
		if (!error)
		{
			result.reset (new btcb::opencl_work (error, config_a, environment, logging_a));
			if (error)
			{
				result.reset ();
			}
